...
#curl -X GET "https://api.cloudflare.com/client/v4/zones/$DNS_ZONE/dns_records?type=A&name=$TARGET_DOMAIN&content=$CUR_IP&proxied=false&page=1&per_page=20&order=type&direction=desc&match=all" \
#    -H "X-Auth-Email: $AUTH_EMAIL" \
#    -H "X-Auth-Key: $AUTH_KEY" \
#    -H "Content-Type: application/json" | jq '.' > temp-record-result
